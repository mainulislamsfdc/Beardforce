    ...
    <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center p-6 relative overflow-hidden">
      ...
      <div className="w-full max-w-md bg-slate-900/80 backdrop-blur-xl border border-slate-800 rounded-2xl shadow-2xl p-8 z-10">
        <div className="text-center mb-8">
            ...
            <p className="text-slate-400 text-sm mt-2">Sign in to your autonomous CRM dashboard.</p>
        </div>
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          ...
          >
            {loading ? <Loader2 className="animate-spin" size={20}/> : <>Enter Dashboard <ArrowRight size={20}/></>}
          </button>
        </form>

        <div className="mt-6 text-center">
            <p className="text-xs text-slate-500">Don't have an account? <span className="text-amber-500 cursor-pointer hover:underline">Contact Administrator</span></p>
        </div>
      </div>
    </div>
  );
